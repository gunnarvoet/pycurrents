...
def _dedup(names):
    ...
    for i in range(1, len(names)):
        if names[i] in names[:i]:
            _log.warning("Duplicate name in CTD file: %s", names[i])
            names[i] = '_' + names[i]
...
class CnvFile:
    ...
    def __init__(self, filename, yearbase=None, edit=True):
        ...
        # Concatenate XML lines into their own single attribute.
        _attrs = []
        _xml = []
        for k, v in attributes:
            if k.strip().startswith('<'):
                ...
                _attrs.append((k.strip(), v.strip()))

        self.header_XML = '\n'.join(_xml)
        self.attributes = dict(_attrs)
        self.ordered_keys = [k for (k, v) in _attrs]
        ...
        try:
            tfmt = "%b %d %Y %H:%M:%S"
            self.datetime = datetime.datetime.strptime(nmea_time, tfmt)
        except ValueError:
    ...
    def scan_dday(self):
        ...
        if self._dday is None:
            ...
            if d.count() != d.size:
                _log.warning("Some scan times were masked and are now NaN;"
    ...
    def NMEA_dday(self):
        ...
        if self._NMEA_dday is None:
            if 'timeQ' not in self.names:
                d = np.empty((len(self.records),), dtype=np.float64)
                d.fill(np.nan)
                _log.warning("This file has no 'timeQ'.")
                self._NMEA_dday = d
            else:
                d = self.records['timeQ'] / 86400.0
                if d.count() != d.size:
                    _log.warning("Some NMEA times were masked and are now NaN;"
    ...
    def _get_array(self):
        if self._array is None:
            ...
            if self.file_type == 'binary':
                data = np.frombuffer(_databytes, '<f4')
            ...
            if self.edit:
                ...
                if glitch.size > 0:
                    _log.warning("Found %s nonmonotonic scan points.", glitch.size)
                    i1 = glitch[0]
                    n0 = data.shape[0]
                    data = data[:i1]
                    _log.warning("Truncated %s at index %s of %s.",
                                        self.filename, i1, n0)
