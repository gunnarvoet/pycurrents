...
Script for plotting u_dday time differences.  This works
on any UHDAS rbin file, but is most useful for messages
that do not have GGA times (such as $HEHDT)

**Usage**
      udday_diagnostics.py --uhdas_dir path/to/cruise/dir [options]
      udday_diagnostics.py [options] *.hdg.rbin
...
**Options**
    ...
    *data source options*
    ...
    (1) usage with wildcard "*.hdg.rbin" needs no other data source options
    ...
    *instrument and message*
    ...
    (2b) use a  sensor_cfg.py file
      --sensor_cfg    choose a specific file to use for all non- gps.rbin files
                       example: /home/adcp/config/sensor_cfg.py  (this is the default)
                       example: /home/data/KM1001c/raw/config/KM1001c_sensor_cfg.py

                       In this case the program will make one plot for every message
                       that is not covered in the ggatime_diagnostics plots
    ...
    *plotting options*
    ...
    --zoomname      'auto', 'zoom', 'fixed' (default)
    ...
    *save/view options*
    ...
    --outdir2      (also save here, but no thumbnails)
...
**Examples**::
   ...
   udday_diagnostics.py --uhdas_dir /home/data/current_cruise --save_fig

   # make one
   udday_diagnostics.py --uhdas_dir lmgould/LMG1804b --instmsg gpsnav:gps  --save_fig --prefix LMG1804b_gps

   udday_diagnostics.py  --zoomname zoom --save_figT

   udday_diagnostics.py  --print_hist  LMG1804b/rbin/gpsnav/*.gps.rbin
...
from pycurrents.adcp.plottime_diagnostics import  plot_udday_times, zoom_udday_fig, save_fig
from pycurrents.adcp.plottime_diagnostics import  make_udday_hist, save_hist
from pycurrents.adcp.plottime_diagnostics import  sensor_cfg_not_dday, find_sensor_cfgs
...
if __name__ == '__main__':
    ...
    parser.add_option("--use_sensor_cfg", dest="use_sensor_cfg",
                      action="store_true",
                      default = False,
               help="reads and parse sensor_cfg.py (inst:msg).")

    parser.add_option("--sensor_file", dest="sensor_file",
                      default = None,
                      help="reads and parse this specific sensor_cfg.py file")
    ...
    parser.add_option("--zoomname", dest="zoomname",
                      default = 'fixed',
                      help="zoom the figure, options: 'auto', 'fixed' (default) or 'zoom'")
    ...
    if len(args) > 0:
        ...
        if options.instmsg: # force this to be one tuple
            ...
            sensor_list = get_cfg(sensor_file)['sensors']
            imtups = sensor_cfg_not_dday(sensor_list)
        _log.debug(imtups)
    ...
    for inst, msg in imtups:
        ...
        if len(filelist) == 0:
            ...
            try:
                ...
                _log.error(f'no data using {globstr}:\nthrew : {e}')
                got_data = False
        ...
        if options.titlestr is None:
            if inst in metanames.keys():
                titlestr = 'UHDAS timestamps from %s %s (%s)  zoom=%s' % (inst, msg, metanames[inst], zoomname)
            else:
                titlestr = 'UHDAS timestamps from %s (%s) zoom=%s' % (inst, msg, zoomname)
        else:
            titlestr=options.titlestr

        f, ax= plot_udday_times(data, inst, titlestr, yearbase=options.yearbase)
        # zoom (or add UTC dates)
        zoom_udday_fig(f, ax, inst, yearbase=options.yearbase, zoomname=zoomname)
        # save
        if options.save_fig:
            save_fig(f, inst, msg=msg, save_dir = outdir, prefix = options.prefix,
                     zoomname=zoomname, thumbnail=options.save_figT)
            if options.outdir2:
                save_fig(f, inst,  msg=msg, save_dir = outdir2, prefix = options.prefix,
                         zoomname=zoomname, thumbnail=False)

        # histogram
        outlist = make_udday_hist(data, inst, titlestr=titlestr)
        if options.save_hist:
            save_hist(outlist, save_dir=outdir, prefix=options.prefix, inst=inst)
            if options.outdir2:
                save_hist(outlist, save_dir=outdir2, prefix=options.prefix, inst=inst)
