...
class PickDirectoryPopUp(QMainWindow):
    def __init__(self, launch_path=os.getcwd(),
        ...
        self.textLabel = CustomLabel('\n'.join([
            "              - Select ADCP data directory - \n",
            "Identify the directory by contents:\n",
            "UHDAS: has directories [raw, rbin, gbin, proc]",
            "VmDAS: has files [*.LTA, *.STA, *.ENR, *.N1R,...]",
            ]),
    ...
    def on_select_dir(self, selected_dir=''):
        ...
        if not selected_dir:
            selected_dir = QFileDialog.getExistingDirectory(
                caption='Select ADCP data directory',
        ...
        self.enr_files, self.lta_files, self.sta_files = list_vmdas_files(
            selected_dir)
        self.raw_dir = glob(os.path.join(self.data_dir, 'raw'))
        self.rbin_dir = glob(os.path.join(self.data_dir, 'rbin'))
        if (not (self.enr_files or self.lta_files or self.sta_files) and
            not (self.raw_dir and self.rbin_dir)):
            msg = '- NO VALID DATA WERE FOUND -\n'
            msg += 'VmDAS dir. must contain either *.ENR, *.LTA or *.STA files\n'
            msg += 'UHDAS dir. must contain "raw" and "rbin" folders\n'
            msg += '- Try again -'
            ...
            self.textLabel.setText("- Wait for the next form to appear -")
            ...
            restore_cursor()
            return
        elif self.raw_dir and self.rbin_dir:
            self.browseButton.setDisabled(True)
            self.textLabel.setText("- Wait for the next form to appear -")
            # N.B.: defining 'parent' is necessary to launch the next form
            next_form = UHDASProcGenForm(uhdas_dir=self.data_dir,
                                         project_path=self.start_path,
                                         parent=self)
            next_form.show()
            self.hide()
            restore_cursor()
